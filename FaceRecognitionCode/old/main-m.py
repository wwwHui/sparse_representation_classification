...
# 读取图片 特征提取
def data_process_yale(yale_dir='../data/CroppedYale', out_dir='../data/Yale-processed',
                      size=(10, 12), png_dir=None,
...
# 数据集划分
def data_split(data_dir, n=3, gabor=True, gabor_fisher_v=False, verbose=True):
    ...
    if gabor:
        ...
        data = np.concatenate((data, data_gabor), axis=1)
    train_index_list = []
    ...
    test_index_list = [i for i in range(len(labels)) if i not in train_list]
...
# 系数表示分类 随机一张图片
def src_example(A, train_label, test_data, test_label):
...
# 稀疏表示分类 所有图片 计算 acc
def sparse_representation_classification(_train_data, train_label, _test_data, test_label, args, verbose=True):
    show_interval = 1
    ...
    A = train_data.T
    # src_example(A, train_label, test_data, test_label)
    count = 0
    err_list = []
...
# 多进程 稀疏表示分类 所有图片 计算 acc
def multiprocessing_src(id, args, count_list):
    print(datetime.datetime.now(), 'process id:', id, 'count list:', count_list)
    res = {}
    for train_count in count_list:
        args = update_train_count(args, train_count)
        train_data, train_label, test_data, test_label = data_split(args.out_dir, n=train_count, gabor=args.gabor,
                                                                    gabor_fisher_v=args.gabor_fv, verbose=False)
        acc = sparse_representation_classification(train_data, train_label, test_data, test_label, args, verbose=99)
        res[train_count] = acc
...
def init_args():
    ...
    parser.add_argument('--data_pre', action='store_true', default=False)
    parser.add_argument('--yale_dir', type=str, default='../data/CroppedYale',
                        help='yale dataset dir')
    parser.add_argument('--png_dir', type=str, default='../data/Yale-png',
                        help='dir to save png file')
    parser.add_argument('--out_dir', type=str, default='../data/Yale-processed',
                        help='dataset processed dir')
    parser.add_argument('--features', type=str, default='gabor-fv',
...
def run():
    ...
    # data_test()
    if args.data_pre:
        data_process_yale(args.yale_dir, args.out_dir, gabor=args.gabor, gabor_fisher_v=args.gabor_fv)

    if args.num_workers == 0:  # 单进程
        train_data, train_label, test_data, test_label = data_split(args.out_dir, n=args.train_count, gabor=args.gabor,
                                                                    gabor_fisher_v=args.gabor_fv)
        sparse_representation_classification(train_data, train_label, test_data, test_label, args)

    else:  # 多进程
        start_index = 5
        max_count = 40
        count_list = [i for i in range(start_index, max_count+1, 5)]
        print(datetime.datetime.now(), '多进程，进程数', args.num_workers)
        count_dict = {}
        ...
        for i in range(args.num_workers):
            ...
            res = pool.apply_async(multiprocessing_src, args=(i, _args, count_dict[i]))
