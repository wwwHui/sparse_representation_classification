...
# 读取图片 特征提取
def data_process_yale(args, yale_dir='../data/CroppedYale', out_dir='../data/Yale-processed',
    ...
    if args.gist:
        data_gist = np.zeros((length, 300))
        gist_helper = utils_gist.GistUtils(ln_orientation=[3, 3, 3, 3], n_block_num=5, n_prefilt=2)
    labels = []
    count = 0
...
def display_loss(loss_list, title_str):
    ...
    ax.set_xlabel('iter')
    ax.set_ylabel('loss')
    plt.savefig('../data/pic-loss/{}.png'.format(title_str))
    plt.show()
