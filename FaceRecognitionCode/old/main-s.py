...
def data_process_yale(yale_dir='../data/CroppedYale', out_dir='../data/Yale-processed',
    ...
    if gabor:
        ...
        if gabor_fisher_v:
            data_gabor_fv = np.zeros((length, 192 * 168, 40))
    if sift:
        ...
        for file in os.listdir(os.path.join(yale_dir, dir)):
            if file.endswith('pgm') and 'Ambient' not in file:
                ...
                if gabor:
                    ...
                    if gabor_fisher_v:
                        img_gabor_fv = np.zeros((192 * 168, 40))
                        i = 0
                        for fre in [0.1, 0.6, 1, 2, 5]:
                            for theta in np.linspace(0, 2 * 3.14, 8, endpoint=False):
...
def data_split(data_dir, n=3, gabor=True, gabor_fisher_v=False, verbose=True):
    ...
    if gabor:
        if gabor_fisher_v:
            ...
            print(datetime.datetime.now(), 'loaded gabor data and label')
...
def init_args():
    ...
    parser.add_argument('--features', type=str, default='gabor',
    ...
    features_list = ['base', 'gabor', 'gabor-fv']
